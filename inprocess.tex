...
first-order insensitive to the nuisance. That correction, however, meets an obstacle
specific to our target. The influence function of $p_t(y)$ contains the generalized
propensity score: a second nuisance, and a conditional density in the treatment coordinate
of exactly the kind the contrastive construction was adopted to avoid. Written in the usual
way, the correction undoes the benefit of the estimator it is meant to correct.
...
Two structural features of this estimand drive everything that follows, and it is worth
naming them now because each leaves a visible trace later. First, the arguments enter
asymmetrically: the covariate is averaged out, whereas $t$ and $y$ are fixed points at which
the density is read off. It is this contrast between an integrated argument and two evaluated
ones that produces the point masses in the influence function of Section~\ref{sec:eif}.
Second, the only unknown is the conditional density $\eta$ --- the covariate distribution is
handled by the empirical distribution of $X_1,\dots,X_n$ --- and its conditioning set $(T,X)$
has dimension $d+p$ while the outcome is scalar. This second asymmetry is what makes standard
estimators inaccurate and dictates the estimator recalled in Section~\ref{sec:setting-mcd}.

The correction of Section~\ref{sec:eif} will also involve the conditional law of the
treatment, which we name here so that it is available when it surfaces.
...
the correction we develop later does not depend on the choice; but the choice fixes the
accuracy, and the configuration is hostile to classical estimators. Their rates fall with the
dimension of the conditioning argument
...
The second measures how far the joint law departs from independence between $(T,X)$ and $Y$,
and equals one exactly when the outcome is unrelated to the conditioning variables; it is this
departure that the classifier learns.
...
draw. In practice one materializes this experiment as a contrastive dataset
...
Two features of the construction return in the correction, and we flag them so that their
reappearance is not a surprise. The negative class is built from the data by recombination, so
the classifier's training law is itself a functional of $F$; this is what makes the influence
function of the fitted classifier depart from the usual formula in
Section~\ref{sec:mcd-if-classifier}. And because the classifier only ever compares observed
vectors with recombined ones, it operates on the support of the joint law rather than on the
ambient space, which is why the construction remains feasible when $d+p$ is large
...
The nuisance error is different, and the difference is the whole point. Because
